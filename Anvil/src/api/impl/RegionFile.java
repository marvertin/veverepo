...
public class RegionFile {
  ...
  private final RegionPosition pozice;


  // pøedpøipravené chunky, jsou zde všechny, ale jen nìkteré jsou naèteny
  private final ChunkHolder[][] chunks = new ChunkHolder[32][32];



  public RegionFile(final File path) {
    // Inicializace obìktù chunbkù
    for (int xc = 0; xc < 32; xc++) {
      for (int zc = 0; zc < 32; zc++) {
        chunks[xc][zc] = new ChunkHolder(this, xc, zc);
      }
    }

    // Zjištìní souøadnic regionu
    pozice = new RegionPosition(path.getName());
   ...
   */
  public synchronized DataInputStream getChunkDataInputStream(final int xc, final int zc) {
    if (outOfBounds(xc, zc)) {
      debugln("READ", xc, zc, "out of bounds");
      return null;
    }

    try {
      final int offset = getOffset(xc, zc);
      if (offset == 0) {
      ...
      if (sectorNumber + numSectors > sectorFree.size()) {
        debugln("READ", xc, zc, "invalid sector");
        return null;
      }
      ...
      if (length > SECTOR_BYTES * numSectors) {
        debugln("READ", xc, zc, "invalid length: " + length + " > 4096 * " + numSectors);
        return null;
      }
      ...
      if (version == VERSION_GZIP) {
        ...
        file.read(data);
        final DataInputStream ret = new DataInputStream(new BufferedInputStream(new GZIPInputStream(new ByteArrayInputStream(data))));
        // debug("READ", x, z, " = found");
        return ret;
      } else if (version == VERSION_DEFLATE) {
        final byte[] data = new byte[length - 1];
        file.read(data);
        final DataInputStream ret = new DataInputStream(new BufferedInputStream(new InflaterInputStream(new ByteArrayInputStream(data))));
        // debug("READ", x, z, " = found");
        return ret;
      }

      debugln("READ", xc, zc, "unknown version " + version);
      return null;
    } catch (final IOException e) {
      debugln("READ", xc, zc, "exception");
      return null;
    }
  }
  ...
  public Block getBlock(final int xbr, final int zbr, final int y) {
    //System.out.println("#########: " + xbr + " - " + zbr);
    final Block block = chunks[xbr / 16][zbr / 16].getBlock(xbr % 16, zbr % 16, y);
    return block;
  }

  public void setBlock(final int xbr, final int zbr, final int y, final Block block) {
    //System.out.println("#########: " + xbr + " - " + zbr);
    chunks[xbr / 16][zbr / 16].setBlock(xbr % 16, zbr % 16, y, block);
  }

  public void vypisSeckci(final int xbr, final int zbr, final int y) {
    chunks[xbr / 16][zbr / 16].vypisSekci(xbr % 16, zbr % 16, y);

  }

  public void save() {
    for (final ChunkHolder[] chunk1 : chunks) {
      for (final ChunkHolder chunk : chunk1) {
        chunk.save();
      }
    }
  }

  public RegionPosition getPozice() {
    return pozice;
  }
