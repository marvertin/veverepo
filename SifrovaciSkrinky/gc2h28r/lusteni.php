...
<form action="sifruj.php" method="POST">
  ...
  c: <input size="35" id="aaa" type="text" name="aaa" autocomplete="off" value="<?php echo $aaa?>" maxlength=30 />
  n: <input size="20" id="bbb" type="text" name="bbb" autocomplete="off" value="<?php echo $bbb?>" maxlength=18 />
...
<? 
      ...
      $casKdySmimPriste = 0;          
      $result = mysql_query("SELECT poradi, aaa, bbb, vvv,  UNIX_TIMESTAMP(reftime), pok, ipadresa FROM pokus WHERE kac = $kac ORDER BY reftime desc",$link);
      if (!$result) {
      ...
      while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
        ...
        if (!$cisloPoslednihoPokusu) {
          ...
          $_SESSION['poslednipokus'] = "$aaa-$bbb-$vvv";
        } 
        ...
        printf("<tr class='$sudolich'><td class='poradi'>%s</td><td class='aaa'>%s</td><td class='bbb'>%s</td><td class='vvv'>%s</td><td class='reftime'>%s</td><td class='zrus'>%s</td>\n",
         $poradi, $aaa, $bbb, $vvv, casek($reftime), $zobrazitRusiciOdkaz ? "<a href='zruspok.php?pok=$pok'>Zrus</a>" : "-");
        if ($admin) {
